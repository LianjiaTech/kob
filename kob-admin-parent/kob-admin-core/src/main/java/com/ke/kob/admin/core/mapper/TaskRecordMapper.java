...
/**
 * mybatis mapper 操作数据库表kob_record_mapper_ 任务记录表
 *
 * @Author: zhaoyuguang
...
public interface TaskRecordMapper {
    ...
    String TABLE = " kob_task_record_${cluster} ";
    ...
    int insertOne(@Param("tr") TaskRecord taskRecord, @Param("cluster") String cluster);
    ...
    int updateStateByTaskUuid(@Param("state") int state, @Param("taskUuid") String taskUuid, @Param("cluster") String cluster);
    ...
    @Select("select " + COLUMN +
            "from " + TABLE +
            "where task_uuid = #{taskUuid} ")
    TaskRecord findByTaskUuidAndCluster(@Param("taskUuid") String taskUuid, @Param("cluster") String cluster);
    ...
    int selectCountExpireTaskRecord(@Param("now") long now, @Param("cluster") String cluster);
    ...
    @Select("select " + COLUMN +
            ...
            "order by id desc " +
            "limit ${start}, ${limit} ")
    List<TaskRecord> selectListExpireTaskRecord(@Param("now") long now, @Param("start") int start, @Param("limit") int limit, @Param("cluster") String cluster);
    ...
    @Update("<script>" +
            ...
            "   where task_uuid = #{taskUuid} " +
            "</script>")
    void updateByTaskUuid(@Param("param") Map<String, Object> param, @Param("taskUuid") String taskUuid, @Param("cluster") String cluster);
    ...
    @Select("<script>" +
            ...
            "</script>")
    int selectCountByParam(@Param("param") Map<String, Object> param, @Param("cluster") String cluster);
    ...
    @Select("select " + COLUMN +
            ...
            "order by id desc " +
            "limit 1")
    TaskRecord selectLastTaskByJobUuid(@Param("jobUuid") String jobUuid, @Param("cluster") String cluster);
}
